...
st.set_page_config(page_title="Git con Los Simpsons: ¡Aprende a lo Springfield!", page_icon="💡", layout="wide")

# Inicio de git, opciones y menus
st.markdown("<h1 style='text-align: center;'>Taller de Git con Los Simpsons: ¡Aprende a lo Springfield!</h1>", unsafe_allow_html=True)
...
def pagina_principal():
    ...
    with cent_co:
        st.image("./img/git.png", caption="Control de versiones con Git & Los Simpsons", use_column_width='auto')
    
    # Título principal con un estilo más llamativo
    st.markdown("<h1 style='text-align: center;'>Bienvenido al Taller Completo de Git & Los Simpsons</h1>", unsafe_allow_html=True)

    # Subtítulos y texto con emojis para hacerlo más atractivo
    st.markdown("""
        ### 🚀 ¿Qué aprenderás?
        - **Comandos Básicos con Ralph Wiggum**: ¡Hola, soy Ralph! Vamos a aprender los comandos básicos de terminal. Te prometo que no habrá nada raro como una cebolla en el parque.
        - **Configuración e Inicialización con Marge Simpson**: Hola, soy Marge. Configurar Git es tan fácil como hornear una tarta de manzana. Prepárate para una configuración suave y sin complicaciones.
        - **Operaciones Básicas con Bart Simpson**: ¡Hey, soy Bart! Vamos a hacer algunos trucos con Git que te harán sentir como el rey del patio de recreo. Prepárate para aprender a clonar, añadir y eliminar archivos.
        - **Ramas y Colaboración con Milhouse Van Houten**: Hola, soy Milhouse. Hacer ramas y colaboraciones es como jugar al escondite con tus amigos. Aprenderemos a gestionar ramas y a colaborar como si estuviéramos organizando una gran fiesta.
        - **Uso Avanzado de Git con Lisa Simpson**: ¡Hola, soy Lisa! Voy a enseñarte técnicas avanzadas como rebase, stash y cherry-pick. Prepárate para un curso intensivo que te hará el genio de Git.
        - **Integración con GitHub con Ned Flanders**: ¡Hola vecino! Soy Ned, y te mostraré cómo conectar Git con GitHub. ¡Okily dokily! Aprenderás a hacer push y pull requests como un vecino modelo.
        - **Resumen del Taller con Edna Krabappel**: ¡Hola, soy Edna! Todos los personajes de Springfield se unirán para un repaso final. No te preocupes, será mucho más divertido que una clase.
        - **Ejercicios Prácticos con la Familia Simpson**: ¡Pongamos en práctica lo aprendido con la ayuda de toda la familia Simpson! Es el momento de aplicar tus conocimientos en ejercicios interactivos.
        - **Feedback con Maggie Simpson**: Incluso Maggie tiene algo que decir, ¿por qué no tú? Comparte tu opinión para ayudarnos a mejorar el taller. 

        ¡Esperamos que disfrutes del taller y que te conviertas en un experto en Git y GitHub con la ayuda de Los Simpsons!
    """)

    # Añadir un botón para empezar el taller
    if st.button("¡Arranquemos el taller y no dejemos ni un donut sin disfrutar! 🍩"):
        # Mostrar imagen de donuts al hacer clic
        st.image("./img/git_homer.jpeg", caption="¡Hora de los donuts! 🍩", use_column_width='auto')
        st.markdown("### ¡Haz clic en una sección del menú y empieza tu aventura en el mundo de Git y GitHub al estilo Springfield!")
